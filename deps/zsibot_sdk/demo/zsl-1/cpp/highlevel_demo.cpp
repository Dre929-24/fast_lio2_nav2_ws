...
int main() {
  ...
  while (true) {
    ...
    if (kbhit()) {
      ...
      switch (ch) {
      // 速度范围：前进后退方向：-3~-0.05/0.05~3.0m/s
      //          侧向方向：0.1~1.0m/s
      //          旋转方向：0.02~3.0 rad/s
      case 'j': // 速度切换
        // 每次按下'j'键时，速度增加0.1m/s
        speed += 0.1;
        // 输出切换后的速度
        if (speed > 3.0) {
          speed = 3.0; // 限制最大速度为3.0m/s
        }
        std::cout << "Speed switched to: " << speed << " m/s" << std::endl;
        break;
      case 'k': // 速度切换
        // 每次按下'k'键时，速度减少0.1m/s
        speed -= 0.1;
        // 输出切换后的速度
        if (speed < -3.0) {
          speed = -3.0; // 限制最小速度为-3.0m/s
        }
        std::cout << "Speed switched to: " << speed << " m/s" << std::endl;
        break;
      case 'w': // 向前 move(vx, vy, yaw_rate) 以1m/s的速度向前移动，
                // 侧向速度vy为零， 旋转速度yaw_rate为零, 使能巡逻模式支持超低速控制，默认为false
        ret = highlevel.move(speed, 0.0, 0.0);
        break;
      ...
      case 'a': // 向左 move(vx, vy, yaw_rate) 以1m/s的速度向左移动，
        ...
        ret = highlevel.move(0.0, speed, 0.0);
        break;
      ...
      case 'q': // 左转 move(vx, vy, yaw_rate) 以1m/s的速度向左转动，
                // 侧向速度vy为零， 前向速度vx为零
        ret = highlevel.move(0.0, 0.0, speed/2);
        break;
      ...
      // 打印当前直线速度，旋转速度（为旋转的一半）
      std::cout << "Current speed: " << speed << " m/s, rotation speed: " << speed / 2 << " rad/s" << std::endl;
    }
